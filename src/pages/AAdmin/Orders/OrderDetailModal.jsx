...
const OrderDetailModal = ({ show, onHide, order }) => {
  ...
  const items = order.sentToSupplier
    ? order.supplierInfo?.items || []
    : order.items || [];
      ...
      <Modal.Body>
        ...
        <Card className="mb-3">
          <Card.Body>
            ...
            ].filter(Boolean).join(', ')} - {order.shippingAddress?.pincode}</p>
                ...
                <ListGroup.Item key={`${item.posterId || item.collectionId}-${index}`}>
                  <div className="d-flex align-items-center">
                    ...
                    <div className="flex-grow-1">
                      ...
                      <p className="mb-0">Finish: {item.finish || 'N/A'}</p>
                      ...
                      ₹{(
                        item.type === 'collection'
                          ? (item.posters || []).reduce((sum, p) => sum + (p.finalPrice || p.price || 0), 0) *
                            (item.quantity || 1) * (1 - (item.collectionDiscount || 0) / 100)
        ...
        <Card>
          <Card.Body>
            ...
            <p><strong>Payment Status:</strong> <Badge bg={
              order.paymentStatus === 'Completed' ? 'success' :
              order.paymentStatus === 'Pending' ? 'warning' : 'danger'
            }>{order.paymentStatus}</Badge></p>
            <p><strong>Pricing Verified:</strong> <Badge bg={order.verified ? 'success' : 'danger'}>
              {order.verified ? 'Verified' : 'Unverified'}
            </Badge></p>
            <p><strong>Order Status:</strong> <Badge bg={
              order.status === 'Delivered' ? 'success' :
              order.status === 'Shipped' ? 'primary' :
              order.status === 'Processing' ? 'warning' : 'danger'
            }>{order.status || 'Not Set'}</Badge></p>

            {order.issues?.length > 0 && (
              <>
                <h6>Pricing Issues:</h6>
                <ul className="text-danger">
                  {order.issues.map((issue, index) => <li key={index}>{issue}</li>)}
                </ul>
              </>
            )}
            ...
            {order.sentToSupplier && (
              <>
                ...
                <p><strong>Supplier Address:</strong> {[
                  order.supplierInfo?.address?.address,
                  order.supplierInfo?.address?.city,
                  order.supplierInfo?.address?.state
                ].filter(Boolean).join(', ')} - {order.supplierInfo?.address?.pincode}</p>
              </>
            )}
          </Card.Body>
        </Card>
